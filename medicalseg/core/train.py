...
def train(model,
    ...
    while iter < iters:
        for data in loader:
            ...
            #print('\n')
            #print('proj_grad_weight',paddle.max(paddle.abs(model.backbone.patch_embed.proj.weight)).numpy())
            #print('proj_grad_weight',paddle.max(paddle.abs(model.backbone.patch_embed.proj.weight.grad)).numpy())
            #
            #print('output_weight',paddle.max(model.output.weight).numpy())
            #print('output_grad',paddle.max(model.output.weight.grad).numpy())
            #for i in range(1,4):
            #for j in range(2):
            #for layer in model.backbone.layers[i].blocks[j].sublayers():
            #if hasattr(layer, 'weight'):
            #print(layer, paddle.max(paddle.abs(layer.weight)))
            #print(paddle.max(paddle.abs(layer.weight.grad)))
            #print('\n')
